...
* @brief	Constructor
******************************************************************************/
__fastcall TFormFindAssets::TFormFindAssets(TComponent* Owner, TDataBanker* const pDB)
	: TForm(Owner)
{
	assert(pDB);
...
void __fastcall TFormFindAssets::Setup()
{
    ...
    {
		ComboBoxCity->Items->Add(Cities[i].m_strName);
    }
	ComboBoxCity->Text = "";
...
* @brief
******************************************************************************/
void __fastcall TFormFindAssets::EditRecord(TVecPtrAssets& Assets, int nGridRow)
{
    assert(m_pDB);
	assert(FormMain);
    ...
    if( pAssets->GetRecordId( Assets[nGridRow], nRecId) )
    {
...
void __fastcall TFormFindAssets::DoQuery()
{
    ...
    if( m_QueryAssets.size() )
    {
		...
		if( nEpgl != aeNone )
        {
			FilterByEpgl(m_QueryAssets, nEpgl, ComboBoxEpglIneq->Text);
        }
											// filter by price
        if( EditValue->Text.Length() )
        {
			unsigned nValue = EditValue->Text.ToInt();
            FilterByValue(m_QueryAssets, nValue, ComboBoxValueIneq->Text);
        }
                                                    // filter by date
        ...
        if( utils::ParseDate(strDate, nYear, nMonth, nDay) )
        {
            ...
            FilterByBuildDate(m_QueryAssets, Date.Val, ComboBoxBuildDateIneq->Text);
        }
